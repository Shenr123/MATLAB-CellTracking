...
% * v3 adds a generalized cell-division detector to findAndTrackCellsNLS3
%   and findAndTrackCellsNLSH2BNew3: previously a "Divided"/daughter link
%   was only ever created as a side effect of the rupture-detection code
%   path. v3 also detects division independent of rupture, guarded by
%   checks (no other already-tracked cell explains either piece, daughter
%   sizes are reasonably symmetric, combined area is conserved relative
%   to the parent) meant to avoid mistaking two separate nearby cells for
%   a division. AutomatedMigrationAnalyzer2 and its helper files are left
...
for s = sections
    ...
    if c >= 0
        ...
        if isAVI || (channels > 2 && ~isnan(h2b))
            findAndTrackCellsNLSH2BNew3
        else
            findAndTrackCellsNLS3
        end
