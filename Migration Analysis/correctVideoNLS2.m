function timePoints = correctVideoNLS2(video, v, section, constr, loc, scaled)
        ...
        if newFrame >= 1 && newFrame <= planes && frame ~= newFrame
            ...
            for i = 1:length(finishedCells)
                if finishedCells(i).Alive(frame) && frame == find(finishedCells(i).Alive, 1, 'last') && isUnreviewedDeath(i)
                    ...
                    flagRects(end + 1) = rectangle('Position', flagPos, 'LineWidth', ls, 'EdgeColor', 'red'); %#ok<AGROW>
            ...
            %box). Re-derived from the raw channel purely for display -
    ...
    function boundaryPts = segmentNucleus(box, frameNum)
        ...
        %the analysis pipeline's own binarization (same sensitivity),
        %cropped to its bounding box. Display only - never used for any
        %tracking decision. Returns the mask boundary in full-resolution
        %[x y] coordinates (same space as BoundingBox/v), or [] if
        %nothing segments out.
        margin = 3;
        ...
        localBw = bwareaopen(imbinarize(imadjust(crop), 'adaptive', 'Sensitivity', 0.4), 20);
        if ~any(localBw(:))
            return
        end
    ...
    function pressButton(source, ~)
        ...
        if strcmp(fig.SelectionType, 'alt') && selectedCell == 0
            ...
            if selectedCell ~= 0
                if isUnreviewedDeath(selectedCell)
