...
while any(j(1) == 'RC')
    ...
    while j(1) == 'C'
        ...
        loc(s, :) = sort(con(:, 2)', 'descend');
        i = size(bg, 2);
        loc(s, :) = [(loc(s, 1) + loc(s, 2))/2+0.7*l (loc(s, 1) + loc(s, 2))/2-0.7*l (loc(s, 3) + loc(s, 4))/2+0.7*l (loc(s, 3) + loc(s, 4))/2-0.7*l (loc(s, 5) + loc(s, 6))/2+0.7*l (loc(s, 5) + loc(s, 6))/2-0.7*l];
