        ...
        for i = 1:length(cellData)
            ...
            for j = find(~unassignedCells)
                if ~any(activeCells(j).Alive((p + channels - 1) / channels) == [2 4]) && activeCells(j).Area((p + channels - 1) / channels) + cellData(i).Area < activeCells(j).Area((p - 1) / channels) * 2 && sum((activeCells(j).Centroid((p + channels - 1) / channels, :) - cellData(i).Centroid) .^ 2) < 0.0055 * l2 ^ 2
                    ...
                    boxX2 = round(min(size(bw, 2), max(activeCells(j).BoundingBox((p + channels - 1) / channels, 1) + activeCells(j).BoundingBox((p + channels - 1) / channels, 3), cellData(i).BoundingBox(1) + cellData(i).BoundingBox(3)) + 1));
                    imtemp = imclose(imclearborder(bw(boxY1:boxY2, boxX1:boxX2)), strel('disk', 1));
                    D = -bwdist(~imtemp);
